...
public class PatientService {
	...
	public Response doAuth(@HeaderParam("authorization") String authString) {
		...
		try {
			if (authString != null && authString != "") {
				System.out
						.println("Authorization String received from the Header is:"
								+ authString);
				doRetrieveSetUserNamePassword(authString);
				return Response.status(200)
						.entity(isUserAuthenticated(userName, passWord))
						.build();
			} else {
				resultJSON
						.setMessage("Unauthorized access:Please check with Administrator");
			...
			resultJSON.setType(Result.ERROR).setMessage(
					"Please contact system administrator for the issue:"
							+ jsonexe.getMessage());
		...
		try {
			...
			if (user != null) {
				...
				if (resultStr != null && resultStr != "") {
					...
					return Response.status(200)
							.entity(UserDAO.addNewUser(user)).build();
	...
	private String checkMandatoryParamRegister(User user) {
		...
		if (user.getPatient() != null) {
			...
			if (patient.getMobile_number() == null
		...
		if (user.getDoctor() != null) {
			...
			if (doctor.getMobile_number() == null
					|| doctor.getMobile_number() == "")
				return "MobileNumber Parameter is missing in Doctor";
			if (doctor.getAlternate_mobile_number() == null
					|| doctor.getAlternate_mobile_number() == "")
		...
		if (user.getAdmin() != null) {
			...
			if (admin.getMobile_number() == null
					|| admin.getMobile_number() == "")
	...
	public Response retreiveHospitalDetails() {
		...
		try {
			...
			return Response.status(200)
					.entity(HospitalDAO.getHospitalDetails()).build();
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response retreiveUserDetails(@PathParam("username") String username,
			@PathParam("password") String password) {
		Result resultJSON = new Result();
		try {
			System.out
					.println("retrieving the User Details for showing before profile update:"
							+ username + ":" + password);
			return Response.status(200)
					.entity(UserDAO.getUserDetails(username, password)).build();
	...
	public Response UpdateUser(final User user) {
		...
		try {
			if (user != null) {
				System.out.println("UserDetails Received:" + user.getUsername()
						+ user.getPassword() + user.getUserId());
				if (user.getUserId() == null || user.getUserId() == "") {
					resultJSON.setType(Result.ERROR).setMessage(
							"User Id is mandatory to perform update");
					return Response.status(200).entity(resultJSON).build();
				}
				if (user.getUserType() != null && user.getUserType() != "") {
					resultJSON
							...
							.setMessage(
									"User Type cannot be updated:Please consider registering another account");
					return Response.status(200).entity(resultJSON).build();
				}

				/*
				 * if(user.getPatient() != null) {
				 * if(user.getPatient().getFirst_name() != null &&
				 * user.getPatient().getFirst_name()!= "") {
				 * resultJSON.setType(Result
				 * .ERROR).setMessage("FirstName of the Patient cannot be updated"
				 * ); return Response.status(200).entity(resultJSON).build(); }
				 * if(user.getPatient().getLast_name() != null &&
				 * user.getPatient().getLast_name()!= "") {
				 * resultJSON.setType(Result
				 * .ERROR).setMessage("LastName of the Patient cannot be updated"
				 * ); return Response.status(200).entity(resultJSON).build(); }
				 * }
				 * 
				 * if(user.getAdmin()!=null) {
				 * if(user.getAdmin().getFirst_name() != null &&
				 * user.getAdmin().getFirst_name()!= "") {
				 * resultJSON.setType(Result
				 * .ERROR).setMessage("FirstName of the Admin cannot be updated"
				 * ); return Response.status(200).entity(resultJSON).build(); }
				 * if(user.getAdmin().getLast_name() != null &&
				 * user.getAdmin().getLast_name()!= "") {
				 * resultJSON.setType(Result
				 * .ERROR).setMessage("LastName of the Admin cannot be updated"
				 * ); return Response.status(200).entity(resultJSON).build(); }
				 * }
				 */
				return Response.status(200).entity(UserDAO.updateUser(user))
						.build();
			}
		} catch (JSONException jsonexe) {
	...
	private void doRetrieveSetUserNamePassword(String authString) {
		...
		try {
			...
			throw new JSONException(
					"Null Pointer Exception at the doRetrieveSetUserNamePassword()");
		}

	}

	private Result isUserAuthenticated(String uname, String pwd)
			throws SQLException {
